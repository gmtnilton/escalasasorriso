...
fun FiltroSheet(
    ...
    ModalBottomSheet(onDismissRequest = onFechar) {
        Column(modifier = Modifier.padding(horizontal = 20.dp).padding(bottom = 24.dp)) {
            ...
            SecaoFiltro(titulo = "Período") {
                val opcoes = listOf(
                    "Todos" to FiltroPeriodo.Todos,
            ...
            if (funcoesDisponiveis.isNotEmpty()) {
                SecaoDropdown(
                    titulo = "Função",
                    valorSelecionado = filtro.funcao,
                    opcoes = funcoesDisponiveis,
                    onSelecionar = onFuncaoChange,
                )
            }
